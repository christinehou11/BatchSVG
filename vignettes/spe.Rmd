...
```{r load svg, echo = FALSE, comment=NA, warning=FALSE, message=FALSE}
res_ranks <- read.csv(
    system.file("extdata","res_ranks.csv",package = "BatchSVG"),
    row.names = 1, check.names = FALSE)
```

```{r view svg, comment=NA, message=FALSE, warning=FALSE}
# res_ranks
dim(res_ranks)
head(res_ranks)[,1:5]
```

To focus on the most significant spatially variable genes (SVGs), we will filter
the dataset to retain only the top 2,000 ranked features (rank $\leq$ 2000) from 
...
Additionally, we will refine our selection to include only genes that appear in
more than one sample (n > 1).
...
```{r read subset, message=FALSE, warning=FALSE, comment=NA, echo=FALSE}
load(system.file("extdata","spe_sub4.rda",package = "BatchSVG"))
spe_sub4
```

```{r sub sample filter, message=FALSE, warning=FALSE, comment=NA}
...
```{r svg nSD two plots, comment=NA, warning=FALSE, message=FALSE}
plots <- svg_nSD(list_batch_df = batch_df, 
...
```{r svg nSD one plot, comment=NA, warning=FALSE, message=FALSE}
plots <- svg_nSD(list_batch_df = batch_df[1], 
...
bias_dev <- biasDetect(list_batch_df = batch_df, threshold = "dev", nSD_dev = 7)
...
bias_rank <- biasDetect(list_batch_df = batch_df, threshold = "rank", 
                        nSD_rank = 6)
...
```{r bias detect both, comment = NA, message=FALSE, warning=FALSE}
bias_both <- biasDetect(list_batch_df = batch_df, threshold = "both",
