...
project. Please read 
[here](https://christinehou11.github.io/humanHippocampus2024/articles/humanHippocampus2024.html)
if interested in `humanHippocampus2024` data package.

(The codes to access the *spe* dataset in `humanHippocampus2024` package will
...
```{r svgs_sub4, message=FALSE, warning=FALSE, comment=NA, eval=FALSE}
...
```{r nrow svgs_sub4, message=FALSE, warning=FALSE, comment=NA, eval=FALSE}
nrow(svgs_sub4)
```

```{r load svgs_sub4, message=FALSE, warning=FALSE, comment=NA, echo=FALSE}
svgs_sub4 <- read.csv(
...
data point color pallete (**plot_pallete**). Default values are provided for 
...
```{r bias detect dev plot, message=FALSE, warning=FALSE, fig.height=6}
...
```{r bias detect rank plot, fig.height=6, message=FALSE, warning=FALSE}
bias_rank$sex$Plot
```
...
# shape default = 16
bias_rank_shape <- biasDetect(list_batch_df = list_batch_df, threshold = "rank", 
    nSD_rank = 6, plot_point_shape = c(2, 18))
...
bias_both_color <- biasDetect(list_batch_df = list_batch_df, threshold = "both", 
    nSD_dev = 7, nSD_rank = 6, plot_pallete = "Greens")
...
the color palletes for both batch effects at the same time.
