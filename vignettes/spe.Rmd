...
```{r subset load, message=FALSE, warning=FALSE, comment=NA, echo=FALSE}
data("spe_sub4")
spe_sub4
```
...
```{r nrow svgs_sub4, message=FALSE, warning=FALSE, comment=NA}
nrow(svgs_sub4)
```
...
```{r bias detect dev plot, comment = NA, message=FALSE, warning=FALSE}
...
```{r bias detect rank plot, comment = NA, message=FALSE, warning=FALSE}
bias_rank$sex$Plot
```
...
```{r both sample id table, comment = NA, message=FALSE, warning=FALSE}
head(bias_both$sample_id$Table)
```

```{r both sample id plot, comment = NA, message=FALSE, warning=FALSE}
bias_both$sample_id$Plot
```

```{r bias detect both sex table, comment = NA, message=FALSE, warning=FALSE}
head(bias_both$sex$Table)
```

```{r bias detect both sex plot, comment = NA, message=FALSE, warning=FALSE}
bias_both$sex$Plot
```
